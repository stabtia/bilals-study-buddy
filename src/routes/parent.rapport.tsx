...
function Rapport() {
  ...
  async function exportPdf() {
    ...
    doc.setFontSize(18);
    doc.text("Rapport de progression — Bilal", 40, y); y += 22;
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Généré le ${now}`, 40, y); y += 24;
    doc.setTextColor(0);

    doc.setFontSize(13);
    doc.text("Vue d'ensemble", 40, y); y += 18;
    doc.setFontSize(11);
    const streak = currentStreak(p.daysCompleted);
    const week = daysThisWeek(p.daysCompleted);
    const weekMin = Math.round(Object.entries(p.dailyTime).reduce((n, [d, s]) => {
    ...
    doc.setFontSize(13);
    doc.text("Résultats par matière", 40, y); y += 18;
    doc.setFontSize(11);
    (["maths", "francais", "sciences", "anglais"] as Subject[]).forEach((s) => {
      ...
      doc.text(`- ${meta.label} : ${p.subjectCounts[s]} sessions, ${ok}/${total} bonnes réponses (${rate}%)`, 50, y);
      y += 16;
    });
    y += 8;

    doc.setFontSize(13);
    doc.text("Compétences acquises", 40, y); y += 18;
    doc.setFontSize(11);
    const got = badges.filter((b) => b.check(p));
    if (got.length === 0) {
      doc.text("Aucun badge débloqué pour le moment.", 50, y); y += 16;
    } else {
      got.forEach((b) => { doc.text(`- ${b.label} — ${b.description}`, 50, y); y += 16; });
    }
    y += 8;

    doc.setFontSize(13);
    doc.text("Recommandations", 40, y); y += 18;
    doc.setFontSize(11);
    recommendations(p).forEach((r) => {
      doc.setFont("helvetica", "bold");
      doc.text(`- ${r.title}`, 50, y); y += 14;
      doc.setFont("helvetica", "normal");
      const wrap = doc.splitTextToSize(r.body, 500);
      doc.text(wrap, 60, y); y += wrap.length * 14 + 4;
