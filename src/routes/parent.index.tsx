...
function ParentDashboard() {
  ...
  const totals = subjects.map((s) => ({
    s, ok: p.correctAnswers[s], ko: p.wrongAnswers[s], sessions: p.subjectCounts[s],
  }));
  ...
  return (
    <div className="space-y-6">
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <KPI label="Temps hebdo" value={`${Math.round(weekMinutes)} min`} sub={`Objectif : ${p.weeklyGoal.minutes} min`} />
              ...
              return (
                <div key={s}>
                  <div className="flex justify-between text-sm">
                    <span className="font-medium text-slate-800">{meta.emoji} {meta.label}</span>
                    <span className="text-slate-500">{sessions} sessions · {ok}/{total} bonnes · {rate}%</span>
                  </div>
                  <div className="h-2 bg-slate-100 rounded-full overflow-hidden mt-1">
                    <div className="h-full rounded-full" style={{ width: `${rate}%`, background: "rgb(37 99 235)" }} />
                  </div>
                </div>
              );
            })}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card title="Recommandations">
          <ul className="space-y-2">
            {recs.map((r, i) => (
              <li key={i} className="p-3 rounded-lg border" style={{
                background: r.tone === "warn" ? "rgb(254 242 242)" : r.tone === "good" ? "rgb(240 253 244)" : "rgb(248 250 252)",
                borderColor: r.tone === "warn" ? "rgb(252 165 165)" : r.tone === "good" ? "rgb(134 239 172)" : "rgb(226 232 240)",
              }}>
      ...
      <Card title="Compétences acquises">
        <div className="flex flex-wrap gap-2">
          {unlockedBadges.length === 0 && <span className="text-sm text-slate-500">Aucun badge pour l'instant.</span>}
          {unlockedBadges.map((b) => (
            <span key={b.id} className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-sm">
              <span>{b.emoji}</span> <span className="text-slate-700">{b.label}</span>
            </span>
          ))}
        </div>
        <div className="mt-4 text-sm text-slate-500">
          <Link to="/parent/historique" className="underline">Voir l'historique complet des sessions →</Link>
        </div>
      </Card>
    </div>
  );
